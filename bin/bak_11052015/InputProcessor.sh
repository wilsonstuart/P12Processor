...
func_getkeys()
{
	...
	then
		...
		for i in $( ls unzip/$order_id/*.zip); do
        		unzip -o  $i   -d  keys/
			log_if_error " unzip failed"
                        if [ "$result" = "0" ];
                        then
                                rm  $i
			        log_if_error "rm $1 failed"
                        fi
		...
		log_pkClientError $result $order_id
...
# Call python script to generate pkcs10 file using the public keys in the directory
